...
export interface SelectedPaymentsExportData {
  payments: any[];
  title?: string;
}

export const exportSelectedPaymentsToExcel = (
  data: SelectedPaymentsExportData
): void => {
  const { payments, title = 'Danh sách hóa đơn được chọn' } = data;
  ...
  const summaryData = [
    { 'Chỉ số': 'Tổng số hóa đơn được chọn', 'Giá trị': totalTransactions },
    { 'Chỉ số': 'Đã thanh toán', 'Giá trị': completedTransactions },
    { 'Chỉ số': 'Chờ thanh toán', 'Giá trị': pendingTransactions },
    { 'Chỉ số': 'Thất bại', 'Giá trị': failedTransactions },
    { 'Chỉ số': 'Tổng doanh thu', 'Giá trị': totalAmount },
  ...
  // Generate filename
  const dateStr = new Date().toISOString().split('T')[0];
  const fileName = `HoaDonDuocChon-${dateStr}.xlsx`;
  ...
  try {
    ...
    throw new Error('Không thể xuất hóa đơn được chọn');
  }
};
